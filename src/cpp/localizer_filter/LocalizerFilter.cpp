...
{
  imu = nh.subscribe(topics::imu_topic, 1000, &LocalizerFilter::imu_callback, this);
  camera = nh.subscribe(topics::camera_topic, 100, &LocalizerFilter::camera_callback, this);
  path_state = nh.advertise<kermit::path_state>(topics::path_state_topic, 1000);
}
