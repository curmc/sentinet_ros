...
/*
 * Copies a rmt defined imu_message into a ros defined imu_message
 */
static void imu_message_copy(kermit::mars_imu* imu, imu_message* measured) {
  float ax = measured->linear_acceleration[0];
  float ay = measured->linear_acceleration[1];
  float az = measured->linear_acceleration[2];

  imu->linear_acceleration.x = ax;
  imu->linear_acceleration.y = ay;
  imu->linear_acceleration.z = az;

  imu->angular_velocity.x = measured->angular_velocity[0];
  imu->angular_velocity.y = measured->angular_velocity[1];
  imu->angular_velocity.z = measured->angular_velocity[2];

  // Update measured pitch and roll
  imu->roll = atan2(-az, -ax);
  imu->pitch = atan2(-ay, -sqrt(ax * ax + az * az));
}


bool Kernel::teensy_callback(float lin, float ang) {
  int ret = teensy_write(&dev, lin, ang);

  if(ret == -1)
    ROS_WARN("Warning, message was not written to serial device");

  imu_message imu = teensy_read(&dev);
  kermit::mars_imu resp;
  
  // Copy imu message into ros message
  imu_message_copy(&resp, &imu);
  ...
  resp.header.frame_id = tf_frame_ids::teensy_1;

  // Publish the imu reading
  pub.publish(resp);
  return ret;
}
