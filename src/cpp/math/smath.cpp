...
orientation aprilRotationMatrixToEulerAngles(const matd_t *R) {
        float sy =
            sqrt(R_at(R, 0, 0) * R_at(R, 0, 0) + R_at(R, 1, 0) * R_at(R, 1, 0));
        ...
        return (orientation){.roll = x, .pitch = y, .yaw = z};
}
...
Mat cvEulerAnglesToRotationMatrix(const orientation &theta) {
        ...
        Mat R_x = (Mat_<double>(3, 3) << 1, 0, 0, 0, cx, -sx, 0, sx, cx);

        Mat R_y = (Mat_<double>(3, 3) << cy, 0, sy, 0, 1, 0, -sy, 0, cy);

        Mat R_z = (Mat_<double>(3, 3) << cz, -sz, 0, sz, cz, 0, 0, 0, 1);
