...
void *increment(struct robot_s *robot, float ang, float lin) {
        ...
        // Linear update
        if (!((robot->options & DISCLINANG) && (int)ang)) {
                float v = lin * robot->drive->speed_increment +
        ...
        // Angular Update
        if (!((robot->options & DISCLINANG) && (int)lin)) {
                float v = ang * robot->drive->speed_increment +
...
struct drive_train create_drive_train(float cruising, float max_speed,
        ...
        struct peripheral *p =
            (struct peripheral *)malloc(sizeof(struct peripheral));
        ...
        attach_dt_callback(&dt,
                           &drive_train_main_callback); // Default increment
