...
#include "kermit/controller/wii/wii_controller.h"

wiimote **scan_wii() {
        wiimote **wiimotes = NULL;
        char c;
        while (!wiimotes) {
                printf("Scanning for wiimotes, enter (s) to stop, any other "
                       "key to keep scanning: ");
                c = getchar();
...
void event_loop(wiimote **wiimotes, struct robot_s *robot,
        ...
        if (wiiuse_poll(wiimotes, MAX_WIIMOTES)) {
                ...
                for (; i < MAX_WIIMOTES; ++i) {
                        ...
                        case WIIUSE_EVENT:
                                ...
                                collect_controller_state(robot, wiimotes[i],
                                                         controller);
                                break;
        ...
        if (robot->options & VERBOSE) {
                ...
                if (robot->options & ADVNCD) {
                        printf("ADVANCED");
                        printf("Settings: "
                               "Variable Speed:  | %d |  --- Verbose:          "
                               "  | %d | --- In "
                               "sync:   | %d | "
                               "NonLinear:       | %d |  --- Discrete Lin Ang: "
                               "  | %d | --- "
                               "Debug:     | %d | ",
                               (robot->options & VAR_SPEED),
                               (robot->options & VERBOSE) >> 1,
                               (robot->options & INSYNC) >> 2,
                               (robot->options & NONLIN) >> 3,
                               (robot->options & DISCLINANG) >> 4,
                               (robot->options & DEBUG) >> 6);
                }

                printf("robot: Angular - %f  Linear - %f",
                       robot->drive->angular_vel, robot->drive->linear_vel);

                printf(
                    "contr: home - %d, plus - %d, minus - %d, A - %d, B - %d, "
                    "ONE - "
                    "%d, TWO - %d up - %d, down - %d, left - %d, right - %d",
                    controller->stick->home.value,
                    controller->stick->plus.value,
                    ...
                    controller->stick->down.value,
                    controller->stick->left.value,
...
void execute_callbacks(struct robot_s *robot, struct controller_s *controller) {
        if (controller->stick) {
                ...
                // Loop through all buttons and if there
                // exists a callback to that button
                // execute it
