...
wiimote **scan_wii() {
  ...
  char c;
  while(!wiimotes){
    printf("Scanning for wiimotes, enter (s) to stop, any other key to keep scanning: ");
    c = getchar();
...
void event_loop(wiimote **wiimotes, struct robot_s *robot,
  ...
  if (robot->options & VERBOSE) {
    ...
    printf("robot: Angular - %f  Linear - %f", robot->drive->angular_vel,
           robot->drive->linear_vel);

    printf("contr: home - %d, plus - %d, minus - %d, A - %d, B - %d, ONE - "
